...
export default function App() {
  ...
  const [frameStyle, setFrameStyle] = useState('macos'); // macos, windows, none
  ...
  return (
    <div className="flex h-screen w-full bg-gray-50 overflow-hidden font-sans text-gray-900">
      ...
      <div className="w-[20%] min-w-[280px] h-full bg-white border-r border-gray-200 p-6 overflow-y-auto flex flex-col gap-8 shadow-sm z-20">
        ...
        {/* Frame Style */}
        <div className="flex flex-col gap-3">
          <label className="text-sm font-semibold text-gray-700 uppercase tracking-wider">Browser Frame</label>
          <div className="flex gap-2">
            <button 
              onClick={() => setFrameStyle('none')} 
              className={`flex-1 py-2 text-sm font-medium rounded-lg border transition-colors ${frameStyle === 'none' ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
            >
              None
            </button>
            <button 
              onClick={() => setFrameStyle('macos')} 
              className={`flex-1 py-2 text-sm font-medium rounded-lg border transition-colors ${frameStyle === 'macos' ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
            >
              macOS
            </button>
            <button 
              onClick={() => setFrameStyle('windows')} 
              className={`flex-1 py-2 text-sm font-medium rounded-lg border transition-colors ${frameStyle === 'windows' ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
            >
              Windows
            </button>
          </div>
